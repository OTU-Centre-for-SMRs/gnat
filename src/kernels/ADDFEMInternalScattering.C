#include "ADDFEMInternalScattering.h"

registerMooseObject("GnatApp", ADDFEMInternalScattering);

InputParameters
ADDFEMInternalScattering::validParams()
{
...
ADDFEMInternalScattering::ADDFEMInternalScattering(const InputParameters & parameters)
  ...
  switch (_quadrature_set.getProblemType())
  {
    ...
    case ProblemType::Cartesian2D:
      ...
      _provided_moment_degree =
          static_cast<unsigned int>(-3 + std::sqrt(9 - 8 * (1 - static_cast<int>(num_coupled))));
...
ADReal
ADDFEMInternalScattering::computeQpResidual()
{

#include "ADFVNuclideActivation.h"

registerMooseObject("GnatApp", ADFVNuclideActivation);

InputParameters
ADFVNuclideActivation::validParams()
{
  ...
  params.addRequiredCoupledVar("group_scalar_fluxes",
                               ...
                               "group.");
  params.addParam<std::vector<MooseFunctorName>>("isotope_number_densities",
...
ADFVNuclideActivation::ADFVNuclideActivation(const InputParameters & parameters)
...
ADReal
ADFVNuclideActivation::computeQpResidual()
{
  ...
  for (unsigned int i = 0u; i < _isotope_number_densities.size(); ++i)
  {
    ...
    res += iso_res * (*_isotope_number_densities[i])(elem_args, 0u);
    iso_res = 0.0;
  }

...
registerMooseObject("GnatApp", EmptyTransportMaterial);

InputParameters
EmptyTransportMaterial::validParams()
{
  auto params = Material::validParams();
  params.addClassDescription("Empty material to declare the required material "
                             "properties for Gnat's particle transport solver. "
                             "All other transport materials must inherit from "
                             "this base class.");
  params.addParam<unsigned int>(
      "num_groups", 0u, "The number of energy groups the energy spectrum is divided into.");
  ...
  params.addParam<bool>(
      "add_heating",
      false,
      "Whether or not this material should provide fission heating cross-sections.");
...
EmptyTransportMaterial::EmptyTransportMaterial(const InputParameters & parameters)
  : Material(parameters),
    _scaling(1.0),
    ...
    _has_heating(_has_fission && getParam<bool>("add_heating")),
    ...
    _mat_chi_f_g(_has_fission ? &declareADProperty<std::vector<Real>>(
                              ...
                              : nullptr),
    _mat_heating_g(_has_heating ? &declareADProperty<std::vector<Real>>(

#include "FissionPowerPostProcessor.h"

#include "metaphysicl/raw_type.h"

registerMooseObject("GnatApp", FissionPowerPostProcessor);

InputParameters
FissionPowerPostProcessor::validParams()
{
  auto params = ElementIntegralPostprocessor::validParams();
  params.addClassDescription(
      "A post-processor that computes the total fission power of the radiation "
      "transport equation (specialized for neutrons).");
...
FissionPowerPostProcessor::FissionPowerPostProcessor(const InputParameters & parameters)
...
Real
FissionPowerPostProcessor::computeQpIntegral()
{
  Real val = 0.0;
  for (unsigned int g = 0u; g < _num_groups; ++g)
    val += MetaPhysicL::raw_value(_kappa_fission[_qp][g]) * (*(_group_scalar_fluxes[g]))[_qp];

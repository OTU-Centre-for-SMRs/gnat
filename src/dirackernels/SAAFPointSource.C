#include "SAAFPointSource.h"

#include "RealSphericalHarmonics.h"

registerMooseObject("GnatApp", SAAFPointSource);

InputParameters
SAAFPointSource::validParams()
{
  auto params = SAAFBaseDiracKernel::validParams();
  params.addClassDescription("Computes the anisotropic point source term for the"
                             "current group and direction of the SAAF discrete ordinates "
                             "transport equation. This kernel should not be exposed to the user, "
...
SAAFPointSource::SAAFPointSource(const InputParameters & parameters)
...
void
SAAFPointSource::addPoints()
{
  addPoint(_source_location);
}

Real
SAAFPointSource::computeQpResidual()
{

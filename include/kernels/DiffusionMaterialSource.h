...
// moments provided by the material system.
class DiffusionMaterialSource : public Kernel
{
public:
  static InputParameters validParams();

  DiffusionMaterialSource(const InputParameters & parameters);
...
protected:
   ...
   * compatibility with materials that provide source moments for the transport solvers. This
   * kernel only uses the 0th degree moment of the external source.
   */
  const ADMaterialProperty<std::vector<Real>> & _source_moments;
}; // class DiffusionMaterialSource
